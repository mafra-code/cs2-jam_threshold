    ...
    public class LocaleKO : IDictionarySource
    {
        ...
        public IEnumerable<KeyValuePair<string, string>> ReadEntries(IList<IDictionaryEntryError> errors, Dictionary<string, int> indexCounts)
        {
            return new Dictionary<string, string>
            {
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.ClearedText)), "도시를 불러온 뒤 이 대체 시스템이 정체로 표시한 오브젝트 수입니다. 숫자가 갱신되지 않은 것 같으면 이 페이지를 다시 여세요." },
                ...
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.BreakdownText)), "종류별" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.BreakdownText)), "같은 합계를 오브젝트 종류별로 나눈 것입니다. 차량에 탑승한 사람은 차량으로 한 번만 셉니다." },
                ...
                { ClearanceStats.BreakdownId, "승용차 {CARS}  ·  트럭 {TRUCKS}  ·  열차 {TRAINS}  ·  대중교통 {TRANSIT}  ·  자전거 {BICYCLES}  ·  보행자 {PEDESTRIANS}  ·  기타 {OTHER}" },
