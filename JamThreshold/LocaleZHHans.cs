    ...
    public class LocaleZHHans : IDictionarySource
    {
        ...
        public IEnumerable<KeyValuePair<string, string>> ReadEntries(IList<IDictionaryEntryError> errors, Dictionary<string, int> indexCounts)
        {
            return new Dictionary<string, string>
            {
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.ClearedText)), "自载入城市以来，本替换判定为堵死的对象数量。若数字看起来没有刷新，请重新打开本页。" },
                ...
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.BreakdownText)), "按类型" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.BreakdownText)), "同一总数按对象类型拆分。车内乘客只通过车辆计一次。" },
                ...
                { ClearanceStats.BreakdownId, "汽车 {CARS}  ·  卡车 {TRUCKS}  ·  列车 {TRAINS}  ·  公共交通 {TRANSIT}  ·  自行车 {BICYCLES}  ·  行人 {PEDESTRIANS}  ·  其他 {OTHER}" },
