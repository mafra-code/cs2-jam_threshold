    ...
    public class LocaleDE : IDictionarySource
    {
        ...
        public IEnumerable<KeyValuePair<string, string>> ReadEntries(IList<IDictionaryEntryError> errors, Dictionary<string, int> indexCounts)
        {
            return new Dictionary<string, string>
            {
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.ClearedText)), "Objekte, die dieser Ersatz seit dem Laden der Stadt als feststeckend markiert hat. Seite neu öffnen, falls die Zahlen veraltet aussehen." },
                ...
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.BreakdownText)), "Nach Typ" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.BreakdownText)), "Dieselbe Summe nach Objekttyp aufgeteilt. Mitfahrende in einem Fahrzeug werden einmal gezählt, über das Fahrzeug." },
                ...
                { ClearanceStats.BreakdownId, "Autos {CARS}  ·  Lkw {TRUCKS}  ·  Züge {TRAINS}  ·  ÖPNV {TRANSIT}  ·  Fahrräder {BICYCLES}  ·  Fußgänger {PEDESTRIANS}  ·  Sonstige {OTHER}" },
